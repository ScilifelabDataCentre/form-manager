<template>
  <div>
    <q-card v-if="Object.keys(urlInfo).length > 0" bordered class="q-my-sm">
      <q-card-section>
        &lt;form action="{{ urlInfo.submission_url }}" method="{{
          urlInfo.method
    ...
    <q-table
      ...
      :columns="
        listingType === 'submission' ? columnsSubmission : columnsQuestion
      "
          ...
          <q-td colspan="100%">
            <q-scroll-area style="height: 250px; max-width: 100%">
              ...
              >
                <thead></thead>
                <tbody>
                  <tr v-for="(value, key) in props.row.submission" :key="key">
              ...
              <q-markup-table v-else flat>
                <thead></thead>
                <tbody>
                  <tr v-for="(value, i) in props.row.submissions" :key="i">
                    <td>
                      {{ value }}
                    </td>
                  </tr>
    ...
    <q-btn
      ...
      @click="
        jsonToClipboard(
          listingType === 'submission' ? rawSubmissions : questions,
        )
      "
    />
      ...
      get() {
        ...
        for (let entry of this.rawSubmissions) {
          for (let key in entry.submission) {
            if (!entry.submission[key].length) continue;
            if (key in tmpData) {
        ...
        for (let key in tmpData)
          outData.push({
            id: key,
            submissions: tmpData[key],
            count: tmpData[key].length,
          });
        return outData;
      },
    },
  ...
  methods: {
    ...
    getEntry() {
      ...
      this.$axios
        ...
        .then(
          (submission) => (this.rawSubmissions = submission.data.submissions),
        )
    ...
    deleteSubmission() {
      this.$axios
        .delete(
          "/api/v1/form/" + this.identifier + "/submission/" + this.toDelete,
