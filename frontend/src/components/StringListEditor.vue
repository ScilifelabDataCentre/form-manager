        ...
        :rules="[
          function (val) {
            return (
              evaluateValue(val) ||
              val.length === 0 ||
              'No whitespace at beginning nor end and must not already exist.'
            );
          },
        ]"
...
export default {
  ...
  methods: {
    ...
    addValue() {
      if (this.enableAdd) {
        ...
        if (!this.modelValue.includes(this.newValue)) {
          this.$emit(
            "update:modelValue",
            this.modelValue.concat(this.newValue),
          );
          this.newValue = "";
        } else this.valueExistsError = true;
      }
    },
    ...
    deleteValue(value) {
      this.$emit(
        "update:modelValue",
        this.modelValue.filter((entry) => entry !== value),
      );
    },
  },
};
