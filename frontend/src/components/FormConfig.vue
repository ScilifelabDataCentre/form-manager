<template>
  <div v-if="Object.keys(editData).length > 0">
    <q-list dense>
      ...
      <q-item>
        <q-item-section>
          <q-input v-model="editData.title" outlined label="Title" />
        </q-item-section>
      </q-item>

      <q-item>
        <q-item-section>
          <q-input v-model="editData.redirect" outlined label="Redirect to" />
        </q-item-section>
      </q-item>
      ...
      <q-item>
        <q-item-section>
          <div class="items-end">
            ...
            />
            <span v-show="editData.saveError" class="text-negative"
              >Save failed</span
            >
          </div>
        </q-item-section>
    ...
    <delete-dialog v-model="showDeleteWarning" @delete-confirmed="deleteForm" />
  </div>
</template>
...
export default defineComponent({
  ...
  methods: {
    ...
    save() {
      ...
      this.$api
        .patch("/form/" + this.identifier, outgoing, {
          headers: { "X-CSRFToken": this.$q.cookies.get("_csrf_token") },
        })
        .then((response) => {
          this.getEntry();
